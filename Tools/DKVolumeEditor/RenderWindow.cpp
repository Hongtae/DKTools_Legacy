...
void RenderWindow::OnKeyDown(wxKeyEvent& e)
{
	int code = e.GetKeyCode();
	switch (code)
	{
	case WXK_SHIFT:
		window->PostKeyboardEvent(DKWindow::EventKeyboardDown, 0, DKVK_LEFT_SHIFT, "", false);
		window->PostKeyboardEvent(DKWindow::EventKeyboardDown, 0, DKVK_RIGHT_SHIFT, "", false);
		break;
	case WXK_CONTROL:
		window->PostKeyboardEvent(DKWindow::EventKeyboardDown, 0, DKVK_LEFT_CONTROL, "", false);
		window->PostKeyboardEvent(DKWindow::EventKeyboardDown, 0, DKVK_RIGHT_CONTROL, "", false);
		break;
	}
}

void RenderWindow::OnKeyUp(wxKeyEvent& e)
{
	int code = e.GetKeyCode();
	switch (code)
	{
	case WXK_SHIFT:
		window->PostKeyboardEvent(DKWindow::EventKeyboardUp, 0, DKVK_LEFT_SHIFT, "", false);
		window->PostKeyboardEvent(DKWindow::EventKeyboardUp, 0, DKVK_RIGHT_SHIFT, "", false);
		break;
	case WXK_CONTROL:
		window->PostKeyboardEvent(DKWindow::EventKeyboardUp, 0, DKVK_LEFT_CONTROL, "", false);
		window->PostKeyboardEvent(DKWindow::EventKeyboardUp, 0, DKVK_RIGHT_CONTROL, "", false);
		break;
	}
}
